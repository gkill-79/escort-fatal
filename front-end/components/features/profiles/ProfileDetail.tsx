...
import {
  MapPin, Star, MessageCircle, Share2, Flag,
  Globe, Ruler, Languages,
} from "lucide-react";
            ...
            <div className="bg-dark-800/80 backdrop-blur rounded-2xl border border-white/8 p-5 space-y-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-white leading-tight">
                  {profile.name}
                  {profile.age != null && <span className="text-dark-400 font-normal text-base ml-2">{profile.age} ans</span>}
                </h1>
                {profile.city && (
                  <p className="flex items-center gap-1.5 text-sm text-dark-400 mt-1">
                    <MapPin className="w-3.5 h-3.5 text-brand-400" />
                    <Link href={`/escorts/${(profile.city as { name: string; slug?: string }).name}`} className="hover:text-brand-400 transition-colors">
                      {profile.city.name}
                    </Link>
                  </p>
                )}
                  ...
                  <div className="flex">
                    {[1,2,3,4,5].map(n => (
                      <Star key={n} className={cn(
                        "w-4 h-4",
                        n <= Math.round(profile.ratingAvg ?? 0)
                          ? "text-yellow-400 fill-yellow-400"
                          : "text-dark-700"
                      )} />
                    ))}
                  </div>
                  <span className="text-sm text-white font-bold">{(profile.ratingAvg ?? 0).toFixed(1)}</span>
                  <span className="text-xs text-dark-500">({profile.ratingCount ?? 0} avis)</span>
                </div>
              )}
              ...
              <div className="pt-2 border-t border-white/5 flex items-center justify-between">
                <button type="button" className="flex items-center gap-1.5 text-xs text-dark-600 hover:text-red-400 transition-colors">
                  <Flag className="w-3 h-3" />
                  Signaler ce profil
                </button>
                <button type="button" className="flex items-center gap-1.5 text-xs text-dark-600 hover:text-brand-400 transition-colors">
